...
// the schema-agnostic sqldump engine over ticket's own tables (from ent
// migrate.Tables). Replaces the legacy per-field ticket.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("ticket/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "ticket", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("ticket: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "ticket", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "ticket", Warnings: res.Warnings}
